...
def setup_conditional_flow(cfg, important_index, device,run, seed,stimuli_index):
    ...
    encoder_params                 = cfg["rnn_encoder"]
    ...
    flow_params["net"]["num_cond_inputs"] = encoder_params["net"]["num_rnn_hidden"] + stimuli_dim + time_dim \
        if stimuli_index is None else encoder_params["net"]["num_rnn_hidden"] + time_dim

    encoder_lr    = encoder_params['lr']
    encoder = rnn_encoder(**encoder_params['net']).to(device)
