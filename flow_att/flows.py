...
def clamp_preserve_gradients(x, min=-5.0, max=3.0):
...
class FlowSequential(nn.Sequential):
    ...
    def log_probs(self, inputs, cond_inputs = None):
        ...
        log_probs = self.base.log_prob(u).sum(-1,keepdim=True)
            
        return (log_probs + log_jacob).sum(-1, keepdim=True)

    def sample(self, num_samples=None, noise=None, cond_inputs=None):
        if noise is None:
            noise = self.base.sample(sample_shape=(cond_inputs.shape[0], self.num_inputs)).float()
            noise = noise
            noise = noise.squeeze(-1) if len(noise.shape) > 2 else noise
